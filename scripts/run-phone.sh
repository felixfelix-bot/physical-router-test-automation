...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -h|--help)
      echo "Usage: ./scripts/run-phone.sh [--no-render] [--run-dir DIR] [extra pytest args...]"
      exit 0
      ;;
...
# --- Directory structure ---
mkdir -p "$RESULTS_DIR/raw/phone" "$RESULTS_DIR/report" "$RESULTS_DIR/artifacts"

# --- Run pytest ---
echo "==> Phone tests: run_id=$RUN_ID"
...
PYTEST_EXIT=0
  ...
  2>&1 | tee "$RESULTS_DIR/raw/phone/output.log" || PYTEST_EXIT=$?
...
if [[ "$NO_RENDER" == "false" ]]; then
  ...
  python3 "$SCRIPT_DIR/collect-results.py" \
    --run-dir "$RESULTS_DIR" \
    --pytest "phone=raw/phone/junit.xml" \
    --run-id "$RUN_ID" \
    ...
    --test-plan "${TOLLGATE_TEST_PLAN:-phone}" \
