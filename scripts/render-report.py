...
def badge(status):
    ...
    bg = status_bg(status)
    label = status.upper()
    return (f'<span style="display:inline-block;font-size:1.1rem;font-weight:700;'
...
def small_badge(status):
    c = status_color(status)
    bg = status_bg(status)
    return (f'<span style="display:inline-block;font-size:0.75rem;font-weight:600;'
...
def render_report(run, summary):
    parts = [
        ...
        render_lab(run),
        render_runners(run),
        render_failed(run, summary),
        render_skipped(run, summary),
        render_native_links(run),
        render_footer(run),
    ]
    body = "\n".join(p for p in parts if p)
    run_id = esc(run.get("run_id", "report"))
    return f"""\
<!DOCTYPE html>
<html lang="en">
<head>
