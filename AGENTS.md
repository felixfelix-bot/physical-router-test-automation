...
**Current state (July 2026)**: The Go backend (`tollgate-module-basic-go`) uses `Origami74/gonuts-tollgate v0.6.1` (V1-only derivation, active fork of `elnosh/gonuts`). V2 keysets from CDK mints are accepted and swapped correctly. **Verified end-to-end on localhost virtual lab** (July 2026): 37 pytest API tests passed against a live CDK V2 mint (keyset `01df97b6fb8a...`), including payment tests that granted active sessions with data allotments. The Go backend starts cleanly with V2 keysets — no crash, no panic. Both V1 (`testnut.cashu.exchange`) and V2 (local CDK mint) can be configured.
